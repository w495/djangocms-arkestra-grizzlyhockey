...
class ForeignKeySearchInput( forms.HiddenInput ):
    ...
    def label_for_value( self, value ):
        ...
        for s in self.search_fields:
            rel_name = s.split( '__' )[0]
            val = getattr( obj, rel_name )
            vals += [val]

        if self.format:
            res = u"" + self.format % tuple(vals)
        else:
            res = u','.join( vals )
        return res

    def __init__( self, rel, arg, attrs = None ):
        self.rel=rel
        if isinstance(arg, tuple):
            self.format = None
            self.search_fields = arg
        elif isinstance(arg, dict):
            self.format = arg['format']
            self.search_fields = arg['fields']
    ...
    def render( self, name, value, attrs = None ):
        ...
        return SafeUnicode( rendered+u'''
            <input type="text" id="lookup_%(name)s" value="%(label)s" size="%(size)s" search_fields="%(search_fields)s" app_label="%(app_label)s" model_name="%(model_name)s" autocomplete_mode="ForeignKey" class="to_autocomplete"/>
...
class ManyToManySearchInput( forms.MultipleHiddenInput ):
    ...
    def label_for_value( self, obj, value ):
        vals = []
        for s in self.search_fields:
            rel_name = s.split( '__' )[0]
            val = getattr( obj, rel_name )
    ...
    def render( self, name, value, attrs = None ):
        ...
        for id in value:
            ...
            selected=selected+mark_safe( u"""
                <div class="to_delete deletelink" ><input type="hidden" name="%(name)s" value="%(value)s"/>%(label)s</div>"""
                    ...
                    'label': self.label_for_value( obj, rel_name ),
                    'name': name,
                    'value': obj.id,
        }
        ...
        return mark_safe( u'''
            ...
            <div style="float:left; padding-left:1px; width:200px;">
...
class AutocompleteInlineModelAdmin( InlineModelAdmin ):
    ...
    def formfield_for_dbfield__( self, db_field, **kwargs ):
        # For ForeignKey use a special Autocomplete widget.
        if isinstance( db_field, models.ForeignKey ) and db_field.name in self.related_search_fields:
            kwargs['widget']=ForeignKeySearchInput( db_field.rel,
                                    self.related_search_fields[db_field.name] )
