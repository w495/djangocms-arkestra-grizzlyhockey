...
class LinksPlugin(CMSPluginBase):
    ...
    def render(self, context, instance, placeholder):
        links = list(instance.links_item.active_items().exclude(destination_object_id=None))  
        if links:
...
class CarouselPluginPublisher(CMSPluginBase):
    ...
    def render(self, context, instance, placeholder):
        segments = list(instance.carousel_item.active_items().exclude(destination_object_id=None))  
        if len(segments) > 1:
